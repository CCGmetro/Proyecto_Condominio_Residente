...
// --- FUNCIÓN DE CARGA INICIAL (ÚNICA Y EFICIENTE) ---
function getInitialAppData(rut) {
  try {
    ...
    const [
      ...
      getResidentPackages(formattedRut),
      getMessages(formattedRut)
    ];
...
function getLaundryServices() {
  try {
    ...
    Logger.log('Error en getLaundryServices: ' + error.stack);
...
function getPublicServicesStatus() {
  try {
    ...
    return { success: false, data: { elevators: [] } };
  }
}
...
function login(rut, password) {
  try {
    ...
    if (resident) {
      ...
      const storedPassword = resident.Password || initialPassword;
...
function getResidentData(rut) {
  try {
    ...
    if (resident) {
      return { success: true, data: {
        rut: resident.Rut || '', name: resident.Nombre || '', lastName: resident.Apellidos || '',
        tower: resident.Torre || '', department: resident.Departamento || '',
        phone: resident.Fono || '', email: resident.Correo || '', photo: resident.Foto || ''
      }};
    }
